...
std::vector<float> Vocoder::infer(const std::vector<std::vector<float>> &mel,
  ...
  try {
    ...
    // Prepare mel input: [batch=1, frames, num_mels]
    // PC-NSF-HiFiGAN expects mel shape [1, n_frames, 128]
    std::vector<int64_t> melShape = {1, static_cast<int64_t>(numFrames),
                                     static_cast<int64_t>(numMels)};
    std::vector<float> melData(numMels * numFrames);

    // Copy mel as [T, num_mels] (no transpose needed — model expects this layout)
    for (size_t frame = 0; frame < numFrames; ++frame) {
      for (int m = 0; m < numMels && m < static_cast<int>(mel[frame].size());
           ++m) {
        melData[frame * numMels + m] = mel[frame][m];
      }
    }

    // Validate mel spectrogram values
    // PC-NSF-HiFiGAN expects mel values in natural-log (ln) domain.
    // Typical speech range is approximately [-12, 2], but ODE outputs
    // may slightly exceed those bounds.
    ...
    // Safety clamp to prevent extreme values / numerical issues
    // ln domain: typical speech mel range is [-12, 2], allow slight margin
    const float melMinClamp = -14.0f;
    const float melMaxClamp = 3.0f;
    for (float &v : melData) {
